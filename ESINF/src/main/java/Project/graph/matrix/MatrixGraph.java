...
import Project.graph.CommonGraph;
...
public class MatrixGraph<V,E> extends CommonGraph<V,E> {
    ...
    public Collection<Edge<V, E>> outgoingEdges(V vert) {
        Collection<Edge<V, E>> edgeCollection = new ArrayList<>();
        int vertKey = key(vert);
        if (vertKey == -1) return edgeCollection;
        Edge<V,E> [] outgoingEdgesArray = edgeMatrix[vertKey];
        for (int i = 0; i < outgoingEdgesArray.length; i++) {
            if (outgoingEdgesArray[i] != null){
                edgeCollection.add(outgoingEdgesArray[i]);
            }
        }
        return edgeCollection;
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < numVerts ; i++)
            for (int j = 0 ; j < numVerts; j++)
                if (edgeMatrix[i][j] != null)
                    sb.append("Desde " + i + " para " + j + "-> "+ edgeMatrix[i][j] + "\n");
